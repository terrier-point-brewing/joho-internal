...
export async function loadClaimPools(
  ...
  const [{ data: allocs }, { data: transfers }, { data: exports_ }, { data: conversions }, { data: entries }, bufferPct] = await Promise.all([
    admin.from("batch_allocations").select("id, batch_id, channel, percentage, written_off_at").in("batch_id", ids),
    admin.from("batch_transfers").select("batch_id, volume_bbl").in("batch_id", ids).in("transfer_type", ["kegging", "canning"]),
  ...
  const produced = sum(transfers as Array<{ batch_id: string; volume_bbl: number | null }> | null, (t) => t.batch_id, (t) => Number(t.volume_bbl ?? 0));
  ...
  for (const b of batches) {
    ...
    out.set(b.id, {
      batch: b, pool, packaged: producedBbl > 0,
...
export async function loadClaimableForPartner(admin: SupabaseClient, partnerId: string): Promise<ClaimableBatch[]> {
  ...
  for (const { batch, pool, readyBy, packaged } of pools.values()) {
    if (pool.claimableBbl <= 0) continue;
 ...
 * is on it), invoice numbers and export billing (pricing lives on the invoice
 * they already hold), notes (written for staff), and every attention flag.
 */
export function toPortalHistory(ledger: LedgerPartner | undefined): PortalDeal[] {
  if (!ledger) return [];
  return ledger.commitments.map((c) => ({
    ...
    deposit: c.channel === "contract_brewing"
      ? { billed_cents: c.totals.deposit_billed_cents, paid_cents: c.totals.deposit_paid_cents - c.totals.deposit_refunded_cents }
      : null,
    shipments: c.shipments
      .filter((s) => s.kind === "shipment")
      .map((s) => ({ date: s.date, volume_bbl: s.volume_bbl, lines: s.lines.map((l) => ({ label: l.variant_label, quantity: l.quantity })) })),
