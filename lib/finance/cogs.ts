 ...
 * as DECIMAL DOLLARS (not integer cents): `total_value_change_usd` and
 * `cost_per_unit_usd`. Consumption/waste is recorded as a NEGATIVE value change, so
...
export interface CostAdjustmentRow {
  ...
  /** Decimal-dollar unit cost, used only when total_value_change_usd is absent. */
 ...
 *
 * Prefers the recorded `total_value_change_usd`; falls back to
 * `quantity × cost_per_unit_usd` only when it is null/undefined. Never mixes units:
...
export function adjustmentCost(row: CostAdjustmentRow): number {
  return Math.abs(
    row.total_value_change_usd ?? (row.quantity ?? 0) * (row.cost_per_unit_usd ?? 0)
  );
}
