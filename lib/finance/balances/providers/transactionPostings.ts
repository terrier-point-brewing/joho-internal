 ...
 * The proration itself lives in ./manualFlowEntries.ts, shared with
 * `manualCorrections` -- the provider that counts the same entries on accounts
 * this one is not a step of. Two implementations of "what is a hand-typed
 * movement worth by this month end" would be two answers that can disagree.
 */
...
export const transactionPostings: BalanceProvider = {
  ...
  kind: "derived",
  // Counts hand-typed movements alongside the feeds -- see sumManualFlowEntries
  // in the Promise.all below.
