...
function poolOf(config: Record<string, unknown>): InventoryPool | null {
  const raw = config[INVENTORY_POOL_KEY];
...
async function fetchRawMaterialsCents(supabase: SupabaseClient): Promise<number> {
...
async function fetchPackagingMaterialsCents(supabase: SupabaseClient): Promise<number> {
...
async function fetchFinishedGoodsCents(supabase: SupabaseClient): Promise<number> {
...
export const inventoryOnHand: BalanceProvider = {
  ...
  async compute(ctx: BalanceContext): Promise<number | null> {
    ...
    return sharedRead(ctx, `inventoryOnHand:${pool}`, () => {
      if (pool === "rawMaterials") return fetchRawMaterialsCents(ctx.supabase);
      if (pool === "packagingMaterials") return fetchPackagingMaterialsCents(ctx.supabase);
      return fetchFinishedGoodsCents(ctx.supabase);
    });
  },
};
