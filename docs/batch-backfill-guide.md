...
5. **`batch_conversions.volume_bbl` stops being "the plan" once `converted_at` is set.** Every other reader filters to pending rows (`!c.converted_at`) or prefers the executed transfer, but the Batch Log's Allocation Plan reads this column directly and labels the row **Converted** — so an executed conversion must carry the volume it actually delivered, not the estimate it was booked at.
6. **Expect the allocation plan to stop summing to 100%.** Allocations are percentages of `volume_bbl`; shrink a conversion and the batch legitimately becomes under-allocated. That warning is the truth, not a regression — never invent allocation to silence it.
7. **Verify against `batch_exhaustion`, not the transfer list.** `remaining_bbl` must land at 0 for every batch you touched, parent and child.
