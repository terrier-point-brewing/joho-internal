...
action to mark it handled. State is persisted in the database so it is shared across
users/devices and survives reload.
...
- The banner lists every expense whose resolved chart-of-accounts `account_number` is
  in the alert set (`5110`, `5120`) **and** whose `inventory_updated_at` is null.
- It is a single aggregate banner: a summary line
  ...
  expandable details table (date · merchant · account · amount) and a **Mark updated**
  action per row, plus **Mark all updated**.
- Marking an expense sets `inventory_updated_at = now()`, which drops it from the
  banner. When no un-handled expenses remain, the banner renders nothing.
...
// Filter to un-handled expenses on the alert accounts, newest first (by accounting_date).
...
covers: account in/out of the set, `inventory_updated_at` null vs set, null
`chart_of_accounts`, ordering, and empty input.

### 2. Migration — `supabase/migrations/<timestamp>_expenses_inventory_updated_at.sql`

Add one nullable column to `public.expenses`:
...
Presence = handled. No `inventory_updated_by`. No new table.

New migration file only — do not edit existing migrations.

### 3. API — `app/api/finance/expenses/route.ts` (extend existing route)

- **GET**: add `inventory_updated_at` to the explicit `.select(...)` column list so the
  client receives it. (The route lists columns explicitly, so this is required — it does
  not flow through automatically.)
- **PATCH**: accept an optional `inventory_updated: boolean` on the request body. When
  `inventory_updated` is present, branch early and perform only that update
  (`inventory_updated_at = new Date().toISOString()` when `true`, `null` when `false` for
  undo), returning `{ id, inventory_updated_at }`. This branch is independent of the
  existing CoA-pin logic so the two concerns don't tangle. Auth is unchanged
  (`requireRole([])`, any authenticated user — at least as permissive as the
  viewer-gated Expenses tab).
...
- `"use client"` component. Props: the flagged expenses (from `selectInventoryAlerts`)
  and an `onMarked(id: string)` callback.
...
- Container is the shared `<Banner>` primitive with an informational tone (not a
  ...
  **Details** toggle (as in `UnrecognizedBanner`), and a table of rows
  (date · merchant · account · amount) each with a **Mark updated** button, plus a
  **Mark all updated** control.
...
- Extend the local `ExpenseRow` type with `inventory_updated_at: string | null`.
...
- `onMarked(id)`: `PATCH /api/finance/expenses` with `{ id, inventory_updated: true }`,
  then set that row's `inventory_updated_at` in local `expenses` state so it drops out of
  the banner (no full reload needed).

## Data flow

```
syncRampExpenses (existing) ──▶ public.expenses (+ inventory_updated_at null)
                                          │
              GET /api/finance/expenses (now selects inventory_updated_at)
                                          │
                    ...
                    selectInventoryAlerts(expenses)  ── pure ──
                         ...
                         <InventoryAlertBanner>
                                          │  Mark updated
                    PATCH { id, inventory_updated:true }
                                          │
              expenses.inventory_updated_at = now()  ──▶ row leaves banner
```
...
- Manual verification in the running app: an expense on 5110/5120 shows in the banner;
  **Mark updated** removes it; reload keeps it removed; an expense on another account
  never appears.
...
| `supabase/migrations/<ts>_expenses_inventory_updated_at.sql` | new — add column |
