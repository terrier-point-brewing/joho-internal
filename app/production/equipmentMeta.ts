...
export const EQ: Record<TankType, {
  label: string;
  headerBg: string;
  border: string;
  badge: string;
  ...
  // deprecated — kept for DB compat with existing data
...
// Only active (non-deprecated) types shown in UI
export const EQ_TYPES = (
  ["fermenter", "brite", "brewhouse", "cold_storage", "kegging", "canning"] as TankType[]
).map((t) => [t, EQ[t]!] as [TankType, NonNullable<typeof EQ[TankType]>]);
