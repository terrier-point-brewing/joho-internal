...
import {
  PLANNING_STAGES, STAGE_LABELS, STAGE_TO_EQ_TYPES,
...
export function AddStagePanel({
  ...
  const pool       = equipment.filter(eq => STAGE_TO_EQ_TYPES[stage]?.includes(eq.type));

  // Auto-select the only available equipment for this stage, without an effect.
  const effectiveEquipmentId = equipmentId || (pool.length === 1 ? pool[0].id : "");

  const conflict = effectiveEquipmentId && start && end
    ? findEquipmentConflict(allScheduleEntries, effectiveEquipmentId, start, end, batchId)
    : null;
  const suggestion = conflict
    ? suggestAlternativeEquipment(pool, allScheduleEntries, start, end, batchId, effectiveEquipmentId)
    : null;
  ...
  async function save() {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        ...(volBbl ? { volume_bbl: Number(volBbl) } : {}),
