...
const SUB_TABS = ["Equipment Templates", "Batch Workflows"] as const;
...
export default function WorkflowsTab({
  ...
  const [subTab, setSubTab] = useState<SubTab>("Equipment Templates");
        ...
        {SUB_TABS.map((t) => (
          <button key={t} onClick={() => setSubTab(t)}
            className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px ${
              subTab === t ? "border-amber-500 text-zinc-100" : "border-transparent text-zinc-500 hover:text-zinc-300"
            }`}
          >
            {t}
          </button>
