...
function shortName(name: string, parentName: string | undefined): string {
  ...
  // Also handle em-dash or space-colon variants
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            {hasChildren && (
              <button
                ...
                className="text-zinc-600 hover:text-zinc-400 w-3 shrink-0 text-[10px]"
              >
                {expanded ? "▾" : "▸"}
              </button>
            )}
...
export default function PLPage() {
  ...
  const sectionMonthTotals = (nodes: TreeNode[]) =>
          ...
          <table className="w-full border-collapse text-xs" style={{ minWidth: `${months.length * 96 + 280}px` }}>
            <thead>
              <tr className="bg-zinc-900 border-b border-zinc-800 sticky top-0 z-10">
                <th className="py-2 px-4 text-left text-[10px] text-zinc-600 uppercase tracking-wider font-medium w-64">
                  Account
                </th>
                {months.map((m, i) => (
