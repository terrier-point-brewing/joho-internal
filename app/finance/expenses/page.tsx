...
export default function ExpensesPage() {
  ...
  async function handleSync() {
    ...
    try {
      ...
      const to   = `${year}-12-31`;
      const res = await fetch(`/api/finance/expenses/sync?from=${from}&to=${to}`, { method: "POST" });
      if (!res.ok) {
      ...
      setSyncMsg(`${r.imported} imported · ${r.mapped} mapped · ${r.new_rules} new accounts (${r.auto_matched_rules} auto-matched)`);
      await loadAll(year);
    } catch (e) {
