...
function SquareSyncPanel() {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-5 space-y-4">
      <div>
        <h2 className="text-sm font-semibold text-zinc-200">Sync from Square</h2>
        <p className="text-xs text-zinc-500 mt-0.5">
          Pulls all Square invoices for the selected year. Safe to re-run — existing records are updated, not duplicated.
        </p>
      </div>
...
function PDFImport() {
  ...
  // editable copies of each payload
      ...
      {step === "review" && (
        <div className="space-y-4">
          <p className="text-xs text-zinc-400">
            Review each parsed invoice. Fields highlighted in amber were not detected — please fill them in.
          </p>
            ...
            return (
              <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {hasWarnings && (
                    <span className="text-[10px] text-amber-400">{file.warnings.length} warning{file.warnings.length > 1 ? "s" : ""}</span>
                  )}
                </div>
                ...
                {hasWarnings && (
                  <div className="space-y-0.5">
                    {file.warnings.map((w, j) => (
                      <p key={j} className="text-[10px] text-amber-400/80">{w}</p>
                    ))}
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                  <Field label="Invoice #" required value={draft.invoice_number}
                    onChange={(v) => updateDraft(i, { invoice_number: v, external_id: v })} />
                  <Field label="Customer" required value={draft.customer_name}
                    onChange={(v) => updateDraft(i, { customer_name: v })} />
                  <Field label="Invoice date" value={draft.invoice_date ?? ""}
                    placeholder="YYYY-MM-DD"
                    onChange={(v) => updateDraft(i, { invoice_date: v || null })} />
                  <Field label="Due date" value={draft.due_date ?? ""}
                    placeholder="YYYY-MM-DD"
                    onChange={(v) => updateDraft(i, { due_date: v || null })} />
                  <div>
                    <label className="block text-[10px] text-zinc-500 mb-1">Status</label>
                    <select value={draft.status}
                      onChange={(e) => updateDraft(i, { status: e.target.value as InvoiceStatus })}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-x-8 gap-y-3">
            {COLUMN_FIELDS.map(({ key, label, required }) => (
              <div key={key} className="flex items-center gap-3">
                <span className="w-36 text-xs text-zinc-400 shrink-0">
                  {label}{required && <span className="text-amber-500 ml-0.5">*</span>}
                </span>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep("upload")}
              className="px-4 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs rounded">Back</button>
      ...
      {step === "done" && result && (
        <div className="space-y-3">
          ...
          <button onClick={reset}
            className="px-4 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs rounded">
            Import another file
          </button>
        </div>
      )}
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 text-zinc-100">
      <div className="shrink-0 px-6 pt-5 pb-4 border-b border-zinc-800">
        <h1 className="text-base font-semibold text-zinc-100">Finance · Import</h1>
        <p className="text-xs text-zinc-500 mt-0.5">Admin only</p>
      </div>

      <div className="flex-1 overflow-auto px-6 py-6 max-w-3xl space-y-8">
