...
export async function POST(req: NextRequest) {
  ...
  const { name, category, supplier_id, partner_id, unit, cost_per_unit_usd, stock_quantity } = body;

  const { data, error } = await supabase
    .from("ingredients")
    .insert({ name, category: category ?? null, supplier_id: supplier_id || null, partner_id: partner_id || null, unit, cost_per_unit_usd: cost_per_unit_usd ?? null, stock_quantity: stock_quantity ?? 0 })
    .select("*, suppliers(company_name), contract_brewing_partners(company_name)")
    .single();
