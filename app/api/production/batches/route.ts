...
export async function POST(req: NextRequest) {
  ...
  const { beer_name, planned_brew_date, volume_bbl, turns, status = "planning", notes, recipe_id } = body;
  ...
  const { data: batch, error: batchErr } = await supabase
    .from("brew_batches")
    .insert({ beer_name, planned_brew_date, volume_bbl, turns, status, notes, recipe_id })
    .select()
    .single();
