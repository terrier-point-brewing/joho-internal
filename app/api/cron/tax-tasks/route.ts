/**
 * Daily cron: keeps `tax_tasks` in sync with every active `tax_schedules` row
 * (idempotent — `ensureTasksForSchedule` upserts on (schedule_id, period_end)
 * with `ignoreDuplicates: true`), then fires the lead-time alert email for any
 * open task that has crossed its `due_date - lead_days` threshold and hasn't
 * been alerted yet (`tasksNeedingAlert` / `markAlerted`, both in
 * lib/tax/tasks.ts).
 *
 * `import "@/lib/tax/parties"` is required for its side effect: each party
 * template registers itself with lib/tax/registry on module load, so
 * `getParty` below can resolve `nc_dor_sales_use` (and any future party).
 */
...
export async function GET(req: NextRequest) {
  ...
  const outcome = await runCronJob("tax-tasks", async () => {
