...
export function ShiftTimeline({ periodId }: { periodId: string }) {
  ...
  // Card height is consistent whether or not tips are shown
  const cardH = hasTippedRows ? "h-16" : "h-10";
  ...
  return (
    <div className="overflow-x-auto">
                    ...
                    <Fragment key={wi}>
                      ...
                      {week.map(d => {
                        ...
                        return (
                          <td key={d} className="px-1 py-1 align-top">
                            {h > 0 ? (
                              <div className={`w-20 ${cardH} rounded-lg px-3 py-2 flex flex-col justify-center gap-1 ${hourCellStyle(h)}`}>
                      ...
                      {multiWeek && (
                        <td className="pl-3 pr-2 py-1 align-top border-l border-zinc-800">
                          {wkHours > 0 ? (
                            <div className={`w-20 ${cardH} rounded-lg px-3 py-2 flex flex-col justify-center gap-1 bg-zinc-800 border border-zinc-700`}>
          ...
          <tr className="border-t-2 border-zinc-700">
            ...
            {weeks.map((week, wi) => {
              ...
              const wkTotalTips = hasTippedRows
                ? week.reduce((s, d) =>
                    s + rows.reduce((rs, r) => rs + (r.daily_tips_cents?.[d] ?? 0), 0), 0)
                : null;
              ...
              return (
                <Fragment key={wi}>
                  {week.map(d => {
                    ...
                    const dayTips  = hasTippedRows
                      ? rows.reduce((s, r) => s + (r.daily_tips_cents?.[d] ?? 0), 0)
                      : null;
                    return (
      ...
      <div className="flex items-center gap-4 mt-4 text-zinc-700 text-xs">
        ...
        {hasTippedRows && (
          <span className="text-emerald-900">Tips estimated from {FREQ_LABELS[tip_pool_frequency]} pool</span>
        )}
      </div>
    </div>
  );
